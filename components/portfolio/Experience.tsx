        ...
        <div className="space-y-6">
          {EXPERIENCE.map((exp, idx) => (
            <motion.div
              ...
              className="group"
            >
              <div className="relative rounded-2xl border border-white/15 bg-gradient-to-br from-slate-950/70 to-black/65 p-6 backdrop-blur transition hover:border-sky-200/35 hover:shadow-[0_0_30px_rgba(125,211,252,0.1)]">
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
                  <div>
                    <h3 className="text-xl font-bold tracking-tight text-sky-100">{exp.role}</h3>
                    <p className="mt-1 font-mono text-sm text-teal-100/90">{exp.org}</p>
                  </div>
                  <span className="mt-2 rounded-lg border border-white/15 bg-white/5 px-2.5 py-1 text-sm font-mono text-gray-300 md:mt-0">
                    {exp.duration}
                  </span>
                </div>
